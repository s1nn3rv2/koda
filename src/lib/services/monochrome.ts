...
export interface MonochromeTrack {
    ...
    release_date?: string; // Some tracks have it top-level
    trackNumber?: number;
    volumeNumber?: number;
    artist?: {
        id: number;
...
class MonochromeService {
    ...
    async searchTracks(query: string, limit: number = 25, offset: number = 0): Promise<MonochromeSearchResponse> {
        const response = await fetch(`${this.baseUrl}/search/?s=${encodeURIComponent(query)}&limit=${limit}&offset=${offset}`);
        if (!response.ok) {
        ...
        if (data.data && Array.isArray(data.data.items)) {
            tracks = { items: data.data.items, totalNumberOfItems: data.data.totalNumberOfItems || data.data.items.length };
        } else if (Array.isArray(data.items)) {
            tracks = { items: data.items, totalNumberOfItems: data.totalNumberOfItems || data.items.length };
        } else {
    ...
    async getStreamUrl(trackId: number): Promise<string> {
        const response = await fetch(`${this.baseUrl}/track/?id=${trackId}&quality=${settingsState.audioQuality}`);
    ...
    async getCoverUrl(trackId: number, preferredSize: number = 1280): Promise<string | null> {
        try {
    ...
    mapToTrack(ot: MonochromeTrack): Track {
        ...
        let releaseDate = ot.streamStartDate || ot.release_date || ot.album.release_date || (ot.album.release_year ? ot.album.release_year.toString() : null);
