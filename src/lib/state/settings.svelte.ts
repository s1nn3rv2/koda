...
class SettingsState {
    ...
    monochromeInstance = $state(browser ? localStorage.getItem("monochrome_instance") || "https://monochrome-api.samidy.com" : "https://monochrome-api.samidy.com");
    audioQuality = $state<AudioQuality>(browser ? (localStorage.getItem("audio_quality") as AudioQuality) || "HIGH" : "HIGH");
    downloadQuality = $state<AudioQuality>(browser ? (localStorage.getItem("download_quality") as AudioQuality) || "HI_RES_LOSSLESS" : "HI_RES_LOSSLESS");
    musicPaths = $state<string[]>(browser ? JSON.parse(localStorage.getItem("music_paths") || "[]") : []);
    lastImportGenre = $state(browser ? localStorage.getItem("last_import_genre") || "" : "");
    lastImportFolder = $state(browser ? localStorage.getItem("last_import_folder") || "" : "");
    ...
    async fetchDynamicInstances() {
        ...
        try {
            ...
            if (data) {
                const api = data.api || [];
                const streaming = new Set((data.streaming || []).map((s: any) => s.url));
                ...
                console.log(`[Settings] Fetched ${this.dynamicInstances.length} dynamic instances`);
            }
        } catch (e) {
