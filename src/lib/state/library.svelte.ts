...
import { monochromeService, type MonochromeTrack } from "$lib/services/monochrome";

class LibraryState {
  selection = $state<Selection>(JSON.parse(localStorage.getItem("library_selection") || '{"type":"all"}'));
  scrollPositions = $state<Record<string, number>>(JSON.parse(localStorage.getItem("library_scroll_positions") || "{}"));
  sortModes = $state<Record<string, { column: SortColumn, direction: SortDirection }>>(JSON.parse(localStorage.getItem("library_sort_modes") || "{}"));
  ...
  allTracksSortColumn = $state<SortColumn>(
    (localStorage.getItem("allTracksSortColumn") as SortColumn) || "release_date"
  );
  ...
  constructor() {
    ...
    $effect.root(() => {
      $effect(() => {
        localStorage.setItem("library_selection", JSON.stringify(this.selection));
        localStorage.setItem("library_scroll_positions", JSON.stringify(this.scrollPositions));
        localStorage.setItem("library_sort_modes", JSON.stringify(this.sortModes));
  ...
  async setupListeners() {
    await TauriService.onScanProgress((progress) => {
      ...
      if (progress.current === progress.total) {
        setTimeout(() => { this.scanProgress = null; }, 3000);
        this.refresh();
      }
    });
  }
  ...
  async selectArtist(artistName: string) {
    try {
      ...
      if (artist) {
        this.selection = { type: "artist", artist };
      }
    } catch (e) {
  ...
  async selectAlbum(albumId: string) {
    try {
      ...
      if (album) {
        this.selection = { type: "album", album };
      }
    } catch (e) {
  ...
  async searchOnline(query: string, limit: number = 25, offset: number = 0) {
    ...
    try {
      const results = await monochromeService.searchTracks(query, limit, offset);

      if (offset === 0) {
        this.onlineSearchResults = results.items || [];
      } else {
        this.onlineSearchResults = [...this.onlineSearchResults, ...(results.items || [])];
      }
